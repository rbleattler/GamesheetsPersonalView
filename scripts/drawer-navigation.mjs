...
let __mhCloseDrawerRoot=()=>{};
function __mhSyncDrawerBack(){const button=document.getElementById('backDrawer');if(!button)return;const open=${ui}.drawer.classList.contains('open');button.hidden=!open;button.title=__mhDrawerStack.length?'Back':'Back to app';button.setAttribute('aria-label',button.title)}
function __mhCaptureDrawer(){const body=document.createDocumentFragment();while(${ui}.drawerBody.firstChild)body.appendChild(${ui}.drawerBody.firstChild);return{title:${ui}.drawerTitle.textContent,sub:${ui}.drawerSub.textContent,body,scrollTop:${ui}.drawer.scrollTop}}
function __mhDrawerBack(){const previous=__mhDrawerStack.pop();if(!previous){__mhCloseDrawerRoot();return} ${ui}.drawerTitle.textContent=previous.title;${ui}.drawerSub.textContent=previous.sub;${ui}.drawerBody.replaceChildren(previous.body);__mhSyncDrawerBack();${ui}.drawer.scrollTop=previous.scrollTop||0}
function ${openName}(e,t,a){if(${ui}.drawer.classList.contains('open'))__mhDrawerStack.push(__mhCaptureDrawer());else __mhDrawerStack=[];${ui}.drawerTitle.textContent=e;${ui}.drawerSub.textContent=t||'';${ui}.drawerBody.innerHTML=a||'';${ui}.backdrop.classList.add('open');${ui}.drawer.classList.add('open');${ui}.drawer.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';__mhSyncDrawerBack();${ui}.drawer.scrollTop=0}
...
code += `;(()=>{__mhCloseDrawerRoot=${closeName};document.addEventListener('click',event=>{const button=event.target?.closest?.('#backDrawer');if(!button)return;event.preventDefault();event.stopImmediatePropagation();__mhDrawerBack()},true);window.MyHockeyHubDrawerNavigation={back:__mhDrawerBack,depth:()=>__mhDrawerStack.length,close:${closeName}};__mhSyncDrawerBack()})();`;

const { code:minified } = await transform(code,{loader:'js',minify:true,target:'es2022'});
await writeFile(appPath,minified);
console.log('Added reliable always-available drawer back navigation.');
